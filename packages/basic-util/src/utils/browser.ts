...
function isGecko(): boolean {
  return !!getUserAgent().match(/gecko/i) && !getUserAgent().includes('khtml')
}
...
function browserType(lang: 'zh-cn' | 'en' = 'en') {
  ...
  const [shell = 'none', shellVs = 'unknown'] = new Map([
    ...
    [testUa(/qihu 360se/g), ['360', 'unknown']], // [360浏览器(无版本),]
    [testUa(/2345explorer/g), ['2345', testVs(/2345explorer\/[\d._]+/g)]], // [2345浏览器,]
    [testUa(/metasr/g), ['sougou', 'unknown']], // [搜狗浏览器(无版本),]
    [testUa(/lbbrowser/g), ['liebao', 'unknown']], // [猎豹浏览器(无版本),]
